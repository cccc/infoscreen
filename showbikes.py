...
class bikeswin:
    def __init__(self, xpos, ypos, width, height):
        ...
        self.table = Table(
            ...
            [
                {
                    "text": lambda col, row, bike, data: (
                        ("#%s" % bike["bike_number"]) if "bike_number" in bike else ""
                    ),
                ...
                {
                    "text": lambda col, row, bike, data: (
                        str(bike["address"] or "") if "address" in bike else ""
                    ),
                    "attributes": [curses.color_pair(0), curses.color_pair(3)],
                    "width": width - 2 - 8 - 5,
                },
                {
                    "text": lambda col, row, bike, data: (
                        ("%dm" % bike["distance"]) if "distance" in bike else ""
                    ),
                    "attributes": [curses.color_pair(0), curses.color_pair(3)],
                    "width": 5,
                },
    ...
    def update(self, data):
        ...
        try:
            ...
            self.win.addstr(
                2, 2, "Something went wrong! " + str(msg), curses.color_pair(1)
            )
